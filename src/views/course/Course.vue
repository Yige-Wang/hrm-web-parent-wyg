	...
	<section>
		...
		<el-table :data="courses" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column label="操作" width="300" fixed="right">
				<template scope="scope">
					<el-button type="default" size="small" @click="handleEditBasic(scope.$index, scope.row)">基本信息</el-button>
		...
		<!--编辑界面-->
		<el-dialog title="编辑" :visible.sync="editFormBasicVisible" :close-on-click-modal="false">
			<el-form :model="editBasicForm" label-width="80px" :rules="editFormRules" ref="editBasicForm">
				<el-form-item label="课程名称" prop="name">
					<el-input v-model="editBasicForm.name" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="适用人群" prop="users">
					<el-input v-model="editBasicForm.users" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="课程类型">
					<el-cascader :props="props" style="width: 100%" clearable
								 v-model="editBasicForm.courseTypeId"
								 :options="editBasicForm"></el-cascader>
				</el-form-item>
				<el-form-item label="课程等级" >
					<!--<el-input v-model="course.grade" auto-complete="off"></el-input>-->
					<el-radio v-for="level in levels" v-model="grade" :label="level.id">{{level.name}}</el-radio>
					<!--<el-radio v-model="grade" v-for="level in levels" : >{{levels.name}}</el-radio>-->
				</el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">
				<el-button @click.native="editBasicForm = false">取消</el-button>
	...
	export default {
		...
		data() {
			return {
				...
				editFormBasicVisible: false,//编辑界面是否显示
				editLoading: false,
				editFormRules: {
					name: [
		...
		methods: {
		    ...
		    getCourseTypes(){
		        this.$http.post("/course/courseType/loadTree")
					.then(res=>{
					    ...
					    console.log(this.courseTypes[1].name);
					})
			},
			...
			//显示编辑界面
			handleEditBasic: function (index, row) {
            	alert("dadas")
				this.editFormBasicVisible = true;
				this.editBasicForm = Object.assign({}, row);
				...
				this.$refs.course.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							//NProgress.start();
							let courseAddVo = Object.assign({}, this.course);
							courseAddVo.grade = this.grade;
							//cascader 级联选择器选中的结果是一个数组，我们只需要提交最后一级的类型
							courseAddVo.courseTypeId = this.course.courseTypeId[this.course.courseTypeId.length-1];
							this.$http.post("/course/course/add",courseAddVo)
								.then(res=>{
